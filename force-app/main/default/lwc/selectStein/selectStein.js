...
export default class SelectStein extends LightningElement {
    @api title;
    @api results;
    @api setSelection;
    @api selectMode;
    ...
    prompt = "Show positive feedback only!";
    output = "";
    ...
    generatePrompt() {
        return "You are given data as a JSON array.  Your task is to filter " +
            "the items in the JSON array based on the question or instructions " +
            "given later.  Output the filtered JSON array using the same format " +
            "as the original data and wrap it with markdown tags (\`\`\`json " + 
            "and \`\`\`)\n\n" + 
    ...
    async run() {
        ...
        try {
            let json = this.parseOuput();  
            this.setSelection(json);          
        }
        catch(e) {
    ...
    parseOuput() {
        ...
        if(m !== null) {
            ...
            throw Error("Failed to parse JSON from LLM output");
        }
    }
